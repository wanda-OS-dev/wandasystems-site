...
## 2026-04-09 - Adding group-focus-visible correctly
**Learning:** `group-focus-visible` only works if the parent `.group` element is actually focusable. A standard `<div>` cannot receive focus via keyboard navigation. Therefore adding `group-focus-visible` to a child element inside a non-focusable `<div>` is dead-code. To make a `<div>` focusable for this effect, it needs `tabindex="0"`. Alternatively, the group should be applied to an interactive element like `<a>` or `<button>`. However, adding `tabindex="0"` to non-interactive content just for styling is an accessibility anti-pattern.
**Action:** Before adding `group-focus-visible`, check if the `.group` element is an interactive element (`a`, `button`, etc.) or has a valid reason to be focusable. If it's a static `<div>` (like a metrics card), do not add `group-focus-visible`.
