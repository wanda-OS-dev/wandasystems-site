...
## 2023-11-09 - Keyboard Parity for Interactive Group Components
**Learning:** When using Tailwind `group-hover` utilities to create complex, multi-element interactive states (like glowing cards or animated logos), keyboard-only users navigating via focus are completely left out of these visual cues unless `group-focus-visible` is explicitly paired with every `group-hover` class. Furthermore, when dynamic styles depend on JavaScript-injected CSS variables (e.g. `var(--mouse-x)`), keyboard focus triggers the state without providing those variables, breaking the visual effect.
**Action:** Always mirror `group-hover:*` with `group-focus-visible:*` on interactive components. When binding styles to JS mouse events, always define a fallback in the CSS (e.g., `var(--mouse-x, 50%)`) to ensure the design degrades gracefully for keyboard users.
