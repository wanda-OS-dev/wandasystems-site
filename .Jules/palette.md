...
## 2024-05-18 - "Learn more" generic links accessibility
**Learning:** Generic link texts like "Learn more" in cards are unhelpful for screen reader users as they lack context without visually associating them with the card's title. The `sr-only` class needs to be used to provide contextual context for screen reader users (e.g., "Learn more about [title]"). This was mentioned in memory: "When using generic link text like 'Learn more' or 'Read more' in mapped card components, always append visually hidden (`sr-only`) context spans (e.g., `<span class="sr-only"> about {title}</span>`) to ensure screen readers announce a complete, contextual phrase."
**Action:** Enhance the generic link in `src/pages/index.astro` inside the `services.map` block to include a screen reader-only contextual text. Also, ensure interactive cards with `group-hover` utilities always have corresponding `group-focus-visible` to support keyboard navigation equivalent to mouse interactions.
