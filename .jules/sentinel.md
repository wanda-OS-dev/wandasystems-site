...
## 2025-04-05 - [Hardcoded Supabase API Key]
**Vulnerability:** A hardcoded Supabase API key was present in an inline script (`<script is:inline>`) in `src/layouts/Layout.astro`. This exposes the secret in the version control system and ships it unnecessarily as a hardcoded static string in the HTML, making it trivial for anyone with source access or inspecting the page to find.
**Learning:** Even for "anon" keys intended for client-side use, hardcoding them directly into templates bypasses environment-specific configuration and violates the principle of keeping configuration (especially secrets) out of the codebase.
**Prevention:** Use Astro's `define:vars` directive to securely inject environment variables (like `import.meta.env.PUBLIC_SUPABASE_ANON_KEY`) from the environment into inline scripts. Always include a runtime check (e.g., `if (!SB_KEY) return;`) to ensure the code fails securely if the environment is misconfigured.
