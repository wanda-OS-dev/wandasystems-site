...
## 2025-02-28 - CSP Misconfiguration during Form Migration
**Vulnerability:** The Content-Security-Policy (CSP) across multiple deployment configurations (`vercel.json`, `netlify.toml`, `public/_headers`) was blocking the new backend webhook (`https://wanda.lazytechlab.de`) because it was configured to allow the legacy service (`https://formspree.io`). Furthermore, `public/_headers` was formatted incorrectly using TOML syntax instead of plain-text, causing security headers to be ignored by some hosting providers.
**Learning:** During backend migrations, updating the API endpoint in the client code is insufficient. Deployment configurations (like CSP) must be updated in lockstep across all hosting environments (Netlify, Vercel) to prevent the browser from blocking legitimate outgoing traffic.
**Prevention:** Whenever migrating an external dependency (like a form backend or analytics service), systematically search the codebase for the legacy URL/domain (e.g., `grep -rnw src -e "old-domain.com"`) and ensure all instances in code, `<link rel="preconnect">` tags, and CSP rules are updated to the new domain. Verify configuration file syntax matches the standard required by the hosting provider.
